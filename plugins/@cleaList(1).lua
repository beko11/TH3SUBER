  ...
  if matches[1]:lower() == 'تنظيف' and matches[2]:lower() == 'الحظر' then
    ...
    local function cleanbl(ext, res)
      if tonumber(res.total_count_) == 0 then -- «Blocklist is empty or maybe Bot is not group's admin»
        return tdcli.sendMessage(ext.chat_id, ext.msg_id, 0,  ' ⚠️ _لا يوجد محضورين في القائمه_ ! , 1,  md )'
      end
      local x = 0
      ...
      return tdcli.sendMessage(ext.chat_id, ext.msg_id, 0, '🎈- _تم تنظيف قائمه حظر المجموعه_ !', 1, 'md')
    end
